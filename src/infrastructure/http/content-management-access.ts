...
export type ContentLoginResult =
...
interface ContentManagementAccessOptions {
  createToken?: () => string;
  now?: () => number;
  sessionTtlMs?: number;
}

export class ContentManagementAccess {
  ...
  public constructor(
    ...
    options: ContentManagementAccessOptions = {},
  ) {
    this.createToken =
  ...
  public login(password: string, clientId: string): ContentLoginResult {
    if (!this.password) return { kind: 'invalid' };
